...
public class OntologyControllerTest {
	...
	@Test
	public void given_OneOntology_When_CorrectParamentersAreSentToCreateUserAccess_Then_TheUserAccessIsCreatedAndReturnedAsJSON()
			throws Exception {
		...
		given(ontologyService.getOntologyById(access.getOntology().getId(), sessionUserId))
				.willReturn(access.getOntology());
		given(ontologyService.getOntologyUserAccessByOntologyIdAndUserId(access.getOntology().getId(),
				access.getUser().getUserId(), sessionUserId)).willReturn(access);
