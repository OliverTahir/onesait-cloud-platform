	   ...
	   function changeQuery(){
		   ...
		   if( targetdb == "rtdb"){
			  ...
			  // IF WE HAVE AN ALREADY SELECTED, WE MAKE THE QUERY BASE BE WITH THE ONE WE HAVE
			   ...
			   if 	   ((option == "sql")     && ( queryKp != null)) { $('#querySql').innerHTML =  queryKp;  $('#querySql').val(queryKp); }
			   ...
			   else if (( option == "native") && ( bdtrType == "relationalOracle") && (queryKp == "null")){ $('#querySql').innerHTML =  "select * from " + base_ontology +" where ROWNUM <= 3;"; $('#querySql').val("select * from " + base_ontology +" where ROWNUM <= 3;"); }
			...
			function createQuerySql(){
				...
				// QUERY ASSEMBLY
		        if( hasCondition && hasGroupBy && hasOrderBy )		  { query = query + queryFields + " from " + ontology + " as c where " + condition + " group by " + queryGroupBy + " order by "+ queryOrderBy;}
				else if( !hasCondition && !hasGroupBy && !hasOrderBy) { query = query + queryFields + " from " + ontology + " as c"; }
				else if( hasCondition  && !hasGroupBy && !hasOrderBy) { query = query + queryFields + " from " + ontology + " as c where " + condition; }
				else if( !hasCondition && hasGroupBy  && !hasOrderBy) { query = query + queryFields + " from " + ontology + " as c group by " + queryGroupBy; }
				else if( hasCondition  && !hasGroupBy && hasOrderBy ) { query = query + queryFields + " from " + ontology + " as c where " + condition + " order by "+ queryOrderBy;}
				else if( !hasCondition && hasGroupBy  && hasOrderBy ) { query = query + queryFields + " from " + ontology + " as c group by " + queryGroupBy + " order by "+ queryOrderBy; }
				else if( hasCondition  && hasGroupBy  && !hasOrderBy) { query = query + queryFields + " from " + ontology + " as c where " + condition + " group by " + queryGroupBy; }
				else if( !hasCondition && !hasGroupBy && hasOrderBy ) { query = query + queryFields + " from " + ontology + " as c order by " + queryOrderBy;}
