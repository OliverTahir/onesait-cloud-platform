...
var OntologyCreateController = function() {
	...
	var validTypes = ["object","string","number","integer","date","timestamp-mongo","timestamp","array","geometry","file","boolean"]; // Valid
																																		// property
																																		// types
			...
			if (object){
				...
				$.each(object, function (propKey, propValue){
					...
					if ( propKey == 'type'){
						...
						if (isFile) { objectType = 'file';  } else if (isGeometry) { objectType = 'geometry'; } else if (isTimestampMongo) { objectType = 'timestamp-mongo'; }else if (isTimestamp) { objectType = 'timestamp'; } else { objectType = propValue; }
...
// ontologyCreateReg.validations.noschema}); return false; }
		...
		if (type == 'timestamp-mongo'){
			...
			// properties[prop] = JSON.parse('{"type":"object", '+ updDesc +' '+
			// updEncryp +'
			// "required":["coordinates","type"],"properties":{"coordinates":{"type":"object","properties":{"latitude":{"type":"number"},"longitude":{"type":"number"}}},"type":{"type":"string","enum":["Point"]}}}');
