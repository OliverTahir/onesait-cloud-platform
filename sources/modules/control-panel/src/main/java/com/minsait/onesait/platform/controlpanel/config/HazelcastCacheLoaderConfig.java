...
public class HazelcastCacheLoaderConfig {
	...
	public HazelcastInstance defaultHazelcastInstanceEmbedded() {
		...
		log.info("Configured Local Cache with data: Name : "+config.getConfigurationFile()+" Instance Name: "+config.getInstanceName()+" Group Name: "+config.getGroupConfig().getName());
	...
	public HazelcastInstance dockerHazelcastInstanceEmbedded() {
		Config config = new ClasspathXmlConfig("hazelcast-docker.xml");
		log.info("Configured Local Cache with data: Name : "+config.getConfigurationFile()+" Instance Name: "+config.getInstanceName()+" Group Name: "+config.getGroupConfig().getName());
		return Hazelcast.newHazelcastInstance(config);
	}
}
