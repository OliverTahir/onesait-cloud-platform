...
public class RestSwaggerReader {
	...
	// private static final String XSOFIA2APIKey = "X-OP-APIKey";
	private static final String XSOFIAEXTENSION = "x-sofia2-extension";

	private static List<String> PRODUCES = new ArrayList<String>(Arrays.asList(MediaType.APPLICATION_JSON,
	...
	// private static List<String> CACHEABLE = new
	// ArrayList<String>(Arrays.asList("false", "true"));
	...
	static {
		 ...
		 * =======
		 *
		 * >>>>>>> develop { "name": "X-OP-APIKey", "in": "header", "required": false,
		 * "type": "string" }
		 */
	}
	...
	private void parse(Swagger swagger, OperacionDTO operacionDTO) {
		...
		for (final ApiQueryParameterDTO apiQueryParameterDTO : queryParams) {
			...
			final Parameter parameter = RestSwaggerReaderHelper.populateParameter(swagger, name, desc, true, condition,
					type, null, splitStringValue(value));
