...
public class ApiFIQL {
	...
	public List<ApiDTO> toApiDTO(List<Api> apis) {
		final List<ApiDTO> apisDTO = new ArrayList<ApiDTO>();
	...
	public ApiDTO toApiDTO(Api api) {
		...
		final ArrayList<OperacionDTO> operacionesDTO = new ArrayList<OperacionDTO>();
		...
		final List<ApiAuthentication> autenticaciones = authenticationRepository.findAllByApi(api);
		if (autenticaciones != null && autenticaciones.size() > 0) {
			autenticacion = autenticaciones.get(0);
			final AutenticacionDTO autenticacionDTO = AuthenticationFIQL.toAutenticacionDTO(autenticacion);

			// Se copian los parámetros
			final Set<ApiAuthenticationParameter> parametros = autenticacion.getApiAuthenticationParameters();
			final ArrayList<ArrayList<AutenticacionAtribDTO>> parametrosDTO = new ArrayList<ArrayList<AutenticacionAtribDTO>>();
			for (final ApiAuthenticationParameter parametro : parametros) {
				// Se copian los atributos
				final ArrayList<AutenticacionAtribDTO> atributosDTO = new ArrayList<AutenticacionAtribDTO>();
				for (final ApiAuthenticationAttribute atrib : parametro.getApiautenticacionattribs()) {
