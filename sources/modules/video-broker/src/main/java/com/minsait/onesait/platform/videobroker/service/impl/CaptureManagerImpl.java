...
public class CaptureManagerImpl implements CaptureManager {
	...
	public void synchronize() {
		...
		executingThreads.entrySet()
				.removeIf(e -> e.getValue().equals(null) || ((VideoProcesor) e.getValue()).isInterrupted());
	...
	private boolean isOntologyAuthorized(Ontology ontology) {
		...
		return cp.getClientPlatformOntologies().stream().map(cpo -> cpo.getOntology()).collect(Collectors.toSet())
				.contains(ontology);
	}
