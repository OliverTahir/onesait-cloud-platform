...
public class PeopleDetector extends FrameProcessor {
	...
	// private DetectionResults lastDetectionResults = new DetectionResults(null,
	// null, null);
	...
	private ProcessType processingType = ProcessType.PEOPLE;
	...
	public VideoProcessorResults process(Mat frame) {
		...
		if (foundLocations.rows() > 0) {
			...
			// BufferedImage image = VideoUtils.MatToBufferedImage(frame);
			// frame0.getContentPane().add(new VideoCamViewer(image));
