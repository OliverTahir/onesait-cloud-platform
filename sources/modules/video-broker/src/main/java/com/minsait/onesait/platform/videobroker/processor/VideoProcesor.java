...
public class VideoProcesor implements Runnable {
			...
			try {
				...
				if (frame.size().area() != 0.0) {
					...
					if (processorResult.isThereResults() || getSentinelResults() == 50) {
						...
						resultsQueue.offer(processorResult);
					} else {
				...
				log.info("Interrupted Thread shutting down: {}", e.getMessage());
				break;
			}
		}
	}
			...
			try {
				...
				if (!frame.empty()) {
					...
					try {
						...
						// log the error
						log.error("Error during the image conversion from frame: {}", e.getMessage());
						// System.err.println("Exception during the image conversion from frame: " + e);
					}
				}
			} catch (final Exception e) {
				// log the error
				log.error("Error during the frame reading: {}", e.getMessage());
				// System.err.println("Exception during the frame reading: " + e);
			}
		}
	...
	//
	// // Common
	// final BlockingQueue<VideoProcessorResults> resultQueue = new
	// LinkedBlockingQueue<VideoProcessorResults>();
	//
	// // VideoProcessor
	//
	// final int cameraIp = 0; // the id of the camera to be used (own webcam
	// default)
	// final int timerStep = 28; // time to get each frame
	// final String user = "usuario";
	// final String password = "1234";
	// final PeopleDetector processor = new PeopleDetector(1.5);
	// final String ontology = "VideoResults";
	// final VideoProcessorParameters params = new VideoProcessorParameters("", "0",
	// timerStep, user, password,
	// processor, resultQueue, ontology);
	// final Thread vp = new Thread(new VideoProcesor(params));
	//
	// System.out.println(" cameraIp: " + cameraIp);
	// System.out.println(" timerStep: " + timerStep);
	...
	// vp.start();
	//
	// System.out.println("Threads actives: " + Thread.activeCount());
	//
