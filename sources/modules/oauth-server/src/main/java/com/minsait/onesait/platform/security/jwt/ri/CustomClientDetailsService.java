...
public class CustomClientDetailsService implements ClientDetailsService {
	...
	public ClientDetails loadClientByClientId(String clientId) throws ClientRegistrationException {
		...
		Collection<String> types = Arrays.asList(grantType.split("\\s*,\\s*"));
		Collection<String> scopeList = Arrays.asList(scopes.split("\\s*,\\s*"));

		App app = appRepository.findOne(clientId);
        ...
        if (!clientId.equals(defaultClientId)) {
	        Set<GrantedAuthority> authorities = app.getAppRoles().stream()
	        		.map(AppRole::getName).map(SimpleGrantedAuthority::new)
