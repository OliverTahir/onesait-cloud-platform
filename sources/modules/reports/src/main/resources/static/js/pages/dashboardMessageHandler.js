...
eventer(messageEvent, function (e) {
    ...
    if (messageString.indexOf('closeGadgetFromIframe') >= 0 || messageString.indexOf('closecreateGadgetFromIframe') >=0  ){
	...
	if(message!=null && messageString.indexOf('createGadgetFromIframe') >= 0){
	    console.log(message);            
    ...
    if (messageString.indexOf('command') >= 0){
		 ...
		 $.ajax({
           ...
           'success': function(data) {
               ...
               if(data.status!=null && data.status === "OK"){
               		angular.element(document.querySelector("dashboard")).controller("dashboard").api.CreateGadget(data.data.type,data.data.id,command.information.gadgetName);
               		console.log("success apiCreateGadget data ok"); 
