...
public class MongoNativeManageDBRepository implements ManageDBRepository {
	...
	@Value("${onesaitplatform.database.mongodb.database:#{null}}")
	@Getter
	@Setter
	...
	public void createIndex(String ontology, String attribute) throws DBPersistenceException {
		...
		final Map<String, Integer> indexKey = new HashMap<String, Integer>();
	...
	public ExportData exportToJson(String ontology, long startDateMillis, String pathToFile)
		...
		if (mongoDbConnector.count(database, ontology, "count(" + query + ")") > 0) {
			...
			if (useAuth) {
				pb = new ProcessBuilder("mongoexport", "--host", server, "--db", database, "--username", username,
						"--password", password, "--collection", ontology, "--query", query, "--out", path,
