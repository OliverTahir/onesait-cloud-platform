...
import com.minsait.onesait.platform.persistence.mongodb.quasar.connector.QuasarMongoDBbHttpImpl;
...
public class MongoBasicOpsDBRepository implements BasicOpsDBRepository {
	...
	public List<BulkWriteResult> insertBulk(String ontology, String schema, List<String> instances, boolean order,
		...
		final ArrayList<String> dataToInsert = new ArrayList<String>(instances.size());
	...
	public MultiDocumentOperationResult updateNative(String collection, String updateQuery, boolean includeIds)
		...
		try {
			...
			int endOfQuery = endOfQuery(updateQuery);
	...
	public String queryNativeAsJson(String ontology, String query, int offset, int limit)
		...
		if (preparedQuery.contains("db." + ontology + ".count(")) {
			...
			return "[ {\"value\": " + Long.toString(this.countNative(ontology, query)) + "}]";
		} else {
	...
	public List<String> findAll(String collection, int limit) throws DBPersistenceException {
		try {
			...
			for (final BasicDBObject obj : mongoDbConnector.findAll(database, collection, 0, limit,
					queryExecutionTimeout)) {
				result.add(obj.toJson());
			}
			return result;
	...
	public String querySQLAsJson(String ontology, String query, int offset) throws DBPersistenceException {
		try {
			...
			log.error("DBPersistenceException:" + e.getMessage());
			throw e;
		} catch (final Exception e) {
			log.error(QUASAR_QUERY_ERROR + query, e);
			throw new DBPersistenceException(QUASAR_QUERY_ERROR + query, e);
