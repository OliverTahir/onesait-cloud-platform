	...
	public String queryNativeAsJson(String ontology, String query) throws DBPersistenceException {
		...
		try {
			...
			if (query.indexOf(".createIndex(") != -1) {
				...
				return "Updated " + mongoRepo.updateNative(ontology, queryContent, false) + " rows";
			} else if (query.indexOf(".remove(") != -1) {
				return "Deleted " + mongoRepo.deleteNative(ontology, queryContent, false) + " rows";
				...
				long number = mongoRepo.deleteNative(ontology, queryContent, false).getCount();
