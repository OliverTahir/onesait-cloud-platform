...
public class Client4IoTBrokerConsole {
	...
	private void invoke(String iotbrokerURL, String ontology, String device, String token) {
		try {
			...
			RestClient client = new RestClient(iotbrokerURL);
			terminal.println("...");
			terminal.println("1. Attemping to connect to " + iotbrokerURL + " " + client.getRestServer());
			client.connect(token, device, device + ":1", true);
			terminal.println("2. Getting all existing instances of ontology " + ontology);
			terminal.println("3. Data: " + client.getAll(ontology).toString().substring(0, 100));
