...
import com.minsait.onesait.platform.router.client.RouterClientGateway;
import com.minsait.onesait.platform.router.service.ClientsConfigFactory;
...
public class RouterFlowManagerService {
	...
	public void executeCrudOperations(Exchange exchange) {
		...
		try {
			...
			log.debug("executeCrudOperations: Exception " + ex.getMessage(), ex);
			compositeModel.setOperationResultModel(ex.getResult());
		} catch (Exception e) {
			exchange.getIn().setHeader(ERROR_STR, "");
			log.debug("executeCrudOperations: Exception " + e.getMessage(), e);
		}
