...
public class AuditFlowManagerService {
	...
	private ExecutorService auditQueueListener;
	...
	@PostConstruct
	public void init() {
		auditQueueListener = Executors.newSingleThreadExecutor();

		auditQueueListener.execute(new Runnable() {
			@Override
			public void run() {
				while (true) {
					try {
						...
						log.error("Interrupted Audit Queue listening", e);
					} catch (final JSONException e) {
						log.error(AUDIT_EVENT_ERROR, e);
					} catch (final RouterCrudServiceException e) {
						log.error(AUDIT_EVENT_ERROR, e);
	...
	@PreDestroy
	public void destroy() {
		auditQueueListener.shutdownNow();
	}
