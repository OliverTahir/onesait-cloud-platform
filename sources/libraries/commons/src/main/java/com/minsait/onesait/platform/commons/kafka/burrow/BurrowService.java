...
public class BurrowService {
	...
	public BurrowConsumerGroupsResponse getAllClientGroups(String cluster) {
		...
		try {
			requestResponse = restTemplate.getForObject(burrowHostUrl + V3_KAFKA + cluster + "/consumer",
					String.class);
		} catch (Exception e) {
	...
	private BurrowConsumerTopicInfoResponse getBurrowConsumerTopicInfo(String cluster, String consumer) {
		...
		try {
			requestResponse = restTemplate
					.getForObject(burrowHostUrl + V3_KAFKA + cluster + "/consumer/" + consumer, String.class);
		} catch (Exception e) {
